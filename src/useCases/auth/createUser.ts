...
import {BusinessErrors, SystemErrors} from '../../errors'
...
export class CreateUser {
  ...
  constructor(
    ...
    @inject(TYPES.CreateUserValidationService) createUserValidationService: CreateUserValidationService,
  ) {
  ...
  async exec (createUserInput: ICreateUserInput): Promise<UserData> {
    const isEmailValid = this.createUserValidationService.validate(createUserInput)
    if(isEmailValid) throw new Error(SystemErrors.INVALID_EMAIL)

    const res = await this.userRepository.create(createUserInput as Required<ICreateUserInput>)
